    ...
    public static decimal? FindCrossoverPoint(List<Candlestick> candlesticks, List<decimal> emaShort, List<decimal> emaLong)
    {
        for (int i = 1; i < candlesticks.Count; i++)
        {
                ...
                (emaShort[i - 1] > emaLong[i - 1] && emaShort[i] < emaLong[i]))
            {
                ...
                // Linear interpolation formula to estimate the exact crossover point
                decimal interpolationFactor = Math.Abs((emaShortA - emaLongA) / ((emaShortB - emaLongB) - (emaShortA - emaLongA)));
                decimal crossoverPoint = closeA + interpolationFactor * (closeB - closeA);
